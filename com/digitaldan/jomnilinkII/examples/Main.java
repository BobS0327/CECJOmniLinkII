...
public class Main {
    ...
    public static String ReadButtonName(Connection c, int buttonnumber) {
        // ZoneStatus[] zonestatus = (ZoneStatus[]) s.getStatuses();
        // int objnum = zonestatus[0].getNumber();
        String ret = "";
        Object m;
        try {
            m = c.reqObjectProperties(Message.OBJ_TYPE_BUTTON, buttonnumber - 1, 1,
                                      ObjectProperties.FILTER_1_NAMED,
                                      ObjectProperties.FILTER_2_AREA_ALL,
                                      ObjectProperties.FILTER_3_ANY_LOAD);
            ret = ((ObjectProperties) m).getName();

        } catch (Exception e) {
            if (e instanceof OmniUnknownMessageTypeException
                    || e instanceof IOException
                    || e instanceof OmniInvalidResponseException
                    || e instanceof OmniNotConnectedException) {
            ...
            c.addNotificationListener(new NotificationListener() {
                ...
                public void otherEventNotification(OtherEventNotifications o) {
                    ...
                    for(int k=0; k<o.getNotifications().length; k++) {
                        ...
                        b1 = ((int) (o.getNotifications())[k] & 0xFF);
